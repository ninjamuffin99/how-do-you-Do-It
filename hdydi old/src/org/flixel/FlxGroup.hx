...
class FlxGroup extends FlxBasic
{
    ...
    override public function destroy() : Void
    {
        if (members != null)
        {
            ...
            as3hx.Compat.setArrayLength(members, 0);
            members = null;
        }
        _sortIndex = null;
    }
    ...
    private function set_maxSize(Size : Int) : Int
    {
        ...
        length = as3hx.Compat.setArrayLength(members, _maxSize);
        return Size;
    }
    ...
    //Don't bother adding an object twice.
    {
        ...
        if (Lambda.indexOf(members, Dynamic) >= 0)
        {
            return Dynamic;
        }
        ...
        if (_maxSize > 0)
        {
            ...
            {
                as3hx.Compat.setArrayLength(members, 2);
            }
            else
            {
                as3hx.Compat.setArrayLength(members, _maxSize);
            }
        }
        else
        {
    ...
    public function recycle(ObjectClass : Class<Dynamic> = null) : FlxBasic
    {
        ...
        if (_maxSize > 0)
        {
            if (length < _maxSize)
            {
                ...
                return add(try cast(Type.createInstance(ObjectClass, []), FlxBasic) catch(e:Dynamic) null);
            }
            else
            {
    ...
    {
        var index : Int = Lambda.indexOf(members, Dynamic);
    ...
    {
        var index : Int = Lambda.indexOf(members, OldObject);
        if ((index < 0) || (index >= members.length))
        {
            return null;
        }
    ...
    public function setAll(VariableName : String, Value : Dynamic, Recurse : Bool = true) : Void
    {
        ...
        while (i < length)
        {
            ...
            if (basic != null)
            {
                ...
                {
                    (try cast(basic, FlxGroup) catch(e:Dynamic) null).setAll(VariableName, Value, Recurse);
                }
                else
                {
    ...
    public function callAll(FunctionName : String, Recurse : Bool = true) : Void
    {
        ...
        while (i < length)
        {
            ...
            if (basic != null)
            {
                if (Recurse && (Std.is(basic, FlxGroup)))
                {
                    (try cast(basic, FlxGroup) catch(e:Dynamic) null).callAll(FunctionName, Recurse);
                }
                else
                {
                    Reflect.field(basic, FunctionName)();
                }
            }
        }
    }
    ...
    public function getRandom(StartIndex : Int = 0, Length : Int = 0) : FlxBasic
    {
        if (Length == 0)
        {
            Length = as3hx.Compat.parseInt(length);
        }
        return try cast(FlxG.getRandom(members, StartIndex, Length), FlxBasic) catch(e:Dynamic) null;
    }
    ...
    {
        length = as3hx.Compat.setArrayLength(members, 0);
    }
