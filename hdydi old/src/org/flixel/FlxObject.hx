...
class FlxObject extends FlxBasic
{
    ...
    /**
		 ...
		 */
    public static inline var PATH_LOOP_BACKWARD : Int = 0x000100;
    /**
    ...
    /**
		 ...
		 */
    private static var _pZero : FlxPoint = new FlxPoint();
    ...
    override public function drawDebug(Camera : FlxCamera = null) : Void
    {
        ...
        var boundingBoxX : Float = x - as3hx.Compat.parseInt(Camera.scroll.x * scrollFactor.x);  //copied from getScreenXY()  
        var boundingBoxY : Float = y - as3hx.Compat.parseInt(Camera.scroll.y * scrollFactor.y);
        boundingBoxX = as3hx.Compat.parseInt(boundingBoxX + (((boundingBoxX > 0)) ? 0.0000001 : -0.0000001));
        boundingBoxY = as3hx.Compat.parseInt(boundingBoxY + (((boundingBoxY > 0)) ? 0.0000001 : -0.0000001));
        var boundingBoxWidth : Int = ((width != as3hx.Compat.parseInt(width))) ? width : width - 1;
        var boundingBoxHeight : Int = ((height != as3hx.Compat.parseInt(height))) ? height : height - 1;
    ...
    /**
		 ...
		 */
    public function followPath(Path : FlxPath, Speed : Float = 100, Mode : Int = PATH_FORWARD, AutoRotate : Bool = false) : Void
    {
        ...
        {
            _pathNodeIndex = as3hx.Compat.parseInt(path.nodes.length - 1);
            _pathInc = -1;
        }
        else
        {
    ...
    private function advancePath(Snap : Bool = true) : FlxPoint
    {
        ...
        else if ((_pathMode & PATH_YOYO) > 0)
        {
            if (_pathInc > 0)
            {
                if (_pathNodeIndex >= path.nodes.length)
                {
                    _pathNodeIndex = as3hx.Compat.parseInt(path.nodes.length - 2);
                    if (_pathNodeIndex < 0)
                    {
                        _pathNodeIndex = 0;
                    }
    ...
    //first check if we need to be pointing at the next node yet
    {
        ...
        else if (Math.sqrt(deltaX * deltaX + deltaY * deltaY) < pathSpeed * FlxG.elapsed)
        {
            node = advancePath();
        }
    ...
    public function overlaps(ObjectOrGroup : FlxBasic, InScreenSpace : Bool = false, Camera : FlxCamera = null) : Bool
    {
        ...
        //Since tilemap's have to be the caller, not the target, to do proper tile-based collisions,{
            ...
            return (try cast(ObjectOrGroup, FlxTilemap) catch(e:Dynamic) null).overlaps(this, InScreenSpace, Camera);
        }
        
        var object : FlxObject = try cast(ObjectOrGroup, FlxObject) catch(e:Dynamic) null;
        if (!InScreenSpace)
        {
    ...
    public function overlapsAt(X : Float, Y : Float, ObjectOrGroup : FlxBasic, InScreenSpace : Bool = false, Camera : FlxCamera = null) : Bool
    {
        ...
        //Since tilemap's have to be the caller, not the target, to do proper tile-based collisions,{
            ...
            var tilemap : FlxTilemap = try cast(ObjectOrGroup, FlxTilemap) catch(e:Dynamic) null;
        ...
        var objectScreenPos : FlxPoint = object.getScreenXY(null, Camera);
        _point.x = X - as3hx.Compat.parseInt(Camera.scroll.x * scrollFactor.x);  //copied from getScreenXY()  
        _point.y = Y - as3hx.Compat.parseInt(Camera.scroll.y * scrollFactor.y);
    ...
    public function getScreenXY(Point : FlxPoint = null, Camera : FlxCamera = null) : FlxPoint
    {
        ...
        Point.x = x - as3hx.Compat.parseInt(Camera.scroll.x * scrollFactor.x);
        Point.y = y - as3hx.Compat.parseInt(Camera.scroll.y * scrollFactor.y);
    ...
    //can't separate two immovable objects
    {
        ...
        {
            return (try cast(Object1, FlxTilemap) catch(e:Dynamic) null).overlapsWithCallback(Object2, separateX);
        }
        if (Std.is(Object2, FlxTilemap))
        {
            return (try cast(Object2, FlxTilemap) catch(e:Dynamic) null).overlapsWithCallback(Object1, separateX, true);
        }
        ...
        //Check if the X hulls actually overlap{
            ...
            if ((obj1rect.x + obj1rect.width > obj2rect.x) && (obj1rect.x < obj2rect.x + obj2rect.width) && (obj1rect.y + obj1rect.height > obj2rect.y) && (obj1rect.y < obj2rect.y + obj2rect.height))
            {
                ...
                if (obj1delta > obj2delta)
                {
                    ...
                    {
                        Object1.touching = Object1.touching | RIGHT;
                        Object2.touching = Object2.touching | LEFT;
                    }
                }
                else if (obj1delta < obj2delta)
                {
                    ...
                    {
                        Object1.touching = Object1.touching | LEFT;
                        Object2.touching = Object2.touching | RIGHT;
                    }
                }
            }
        }
    ...
    //can't separate two immovable objects
    {
        ...
        if (Std.is(Object1, FlxTilemap))
        {
            return (try cast(Object1, FlxTilemap) catch(e:Dynamic) null).overlapsWithCallback(Object2, separateY);
        }
        if (Std.is(Object2, FlxTilemap))
        {
            return (try cast(Object2, FlxTilemap) catch(e:Dynamic) null).overlapsWithCallback(Object1, separateY, true);
        }
        ...
        //Check if the Y hulls actually overlap{
            ...
            if ((obj1rect.x + obj1rect.width > obj2rect.x) && (obj1rect.x < obj2rect.x + obj2rect.width) && (obj1rect.y + obj1rect.height > obj2rect.y) && (obj1rect.y < obj2rect.y + obj2rect.height))
            {
                ...
                if (obj1delta > obj2delta)
                {
                    ...
                    {
                        Object1.touching = Object1.touching | DOWN;
                        Object2.touching = Object2.touching | UP;
                    }
                }
                else if (obj1delta < obj2delta)
                {
                    ...
                    {
                        Object1.touching = Object1.touching | UP;
                        Object2.touching = Object2.touching | DOWN;
                    }
                }
            }
        }
