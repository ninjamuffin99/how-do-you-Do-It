...
class FlxU
{
    ...
    {
        var number : Float = as3hx.Compat.parseInt(Value);
    ...
    {
        var number : Float = as3hx.Compat.parseInt(Value + (((Value > 0)) ? 0.5 : -0.5));
        return ((Value > 0)) ? (number) : (((number != Value)) ? (number - 1) : (number));
    }
    ...
    {
        return ((69621 * as3hx.Compat.parseInt(Seed * 0x7FFFFFFF)) % 0x7FFFFFFF) / 0x7FFFFFFF;
    }
    ...
    public static function shuffle(Objects : Array<Dynamic>, HowManyTimes : Int) : Array<Dynamic>
    {
        ...
        {
            index1 = as3hx.Compat.parseInt(Math.random() * Objects.length);
            index2 = as3hx.Compat.parseInt(Math.random() * Objects.length);
    ...
    public static function getRandom(Objects : Array<Dynamic>, StartIndex : Int = 0, Length : Int = 0) : Dynamic
    {
        if (Objects != null)
        {
            ...
            if (l > 0)
            {
                return Objects[StartIndex + as3hx.Compat.parseInt(Math.random() * l)];
            }
        }
        return null;
    }
    ...
    /**
		 ...
		 */
    public static function formatTicks(StartTicks : Int, EndTicks : Int) : String
    {
    ...
    /**
		 ...
		 */
    public static function makeColor(Red : Int, Green : Int, Blue : Int, Alpha : Float = 1.0) : Int
    {
        return as3hx.Compat.parseInt(((((Alpha > 1)) ? Alpha : (Alpha * 255)) & 0xFF) << 24 | (Red & 0xFF) << 16 | (Green & 0xFF) << 8 | (Blue & 0xFF));
    }
    ...
    /**
		 ...
		 * @return	The color as a <code>uint</code>.
		 */
    public static function makeColorFromHSB(Hue : Float, Saturation : Float, Brightness : Float, Alpha : Float = 1.0) : Int
    {
        var red : Float;
        var green : Float;
        var blue : Float;
        ...
        else
        {
            ...
            var slice : Int = as3hx.Compat.parseInt(Hue / 60);
            ...
            switch (slice)
            {
                case 0:red = Brightness;green = cc;blue = aa;
                case 1:red = bb;green = Brightness;blue = aa;
                case 2:red = aa;green = Brightness;blue = cc;
                case 3:red = aa;green = bb;blue = Brightness;
                case 4:red = cc;green = aa;blue = Brightness;
                case 5:red = Brightness;green = aa;blue = bb;
                default:red = 0;green = 0;blue = 0;
            }
        }
        
        return as3hx.Compat.parseInt(((((Alpha > 1)) ? Alpha : (Alpha * 255)) & 0xFF) << 24 | as3hx.Compat.parseInt(red * 255) << 16 | as3hx.Compat.parseInt(green * 255) << 8 | as3hx.Compat.parseInt(blue * 255));
    }
    ...
    /**
		 ...
		 */
    public static function getRGBA(Color : Int, Results : Array<Dynamic> = null) : Array<Dynamic>
    {
        if (Results == null)
        {
            Results = new Array<Dynamic>();
        }
        Results[0] = as3hx.Compat.parseInt(Color >> 16) & 0xFF;
        Results[1] = as3hx.Compat.parseInt(Color >> 8) & 0xFF;
        Results[2] = Color & 0xFF;
        Results[3] = as3hx.Compat.parseFloat(as3hx.Compat.parseInt(Color >> 24) & 0xFF) / 255;
        return Results;
    }
    ...
    /**
		 ...
		 * @return	An <code>Array</code> object containing the Red, Green, Blue and Alpha values of the given color.
		 */
    public static function getHSB(Color : Int, Results : Array<Dynamic> = null) : Array<Dynamic>
    {
        if (Results == null)
        {
            Results = new Array<Dynamic>();
        ...
        var red : Float = as3hx.Compat.parseFloat(as3hx.Compat.parseInt(Color >> 16) & 0xFF) / 255;
        var green : Float = as3hx.Compat.parseFloat(as3hx.Compat.parseInt(Color >> 8) & 0xFF) / 255;
        var blue : Float = as3hx.Compat.parseFloat((Color) & 0xFF) / 255;
    ...
    {
        var timeString : String = as3hx.Compat.parseInt(Seconds / 60) + ":";
        var timeStringHelper : Int = as3hx.Compat.parseInt(as3hx.Compat.parseInt(Seconds) % 60);
        ...
        {
            timeString += ".";
            timeStringHelper = as3hx.Compat.parseInt((Seconds - as3hx.Compat.parseInt(Seconds)) * 100);
            if (timeStringHelper < 10)
            {
                timeString += "0";
            }
    ...
    public static function formatArray(AnyArray : Array<Dynamic>) : String
    {
        ...
        var string : String = Std.string(AnyArray[0]);
        var i : Int = 0;
    ...
    {
        var helper : Int;
        var amount : Int = as3hx.Compat.parseInt(Amount);
        var string : String = "";
        var comma : String = "";
        var zeroes : String = "";
        while (amount > 0)
        {
            ...
            zeroes = "";
            helper = as3hx.Compat.parseInt(amount - as3hx.Compat.parseInt(amount / 1000) * 1000);
            amount /= 1000;
            if (amount > 0)
            {
                if (helper < 100)
        ...
        if (ShowDecimal)
        {
            amount = as3hx.Compat.parseInt(as3hx.Compat.parseInt(Amount * 100) - (as3hx.Compat.parseInt(Amount) * 100));
    ...
    {
        return Type.getClass(Type.resolveClass(Name));
    }
    ...
    /**
		 ...
		 */
    public static function rotatePoint(X : Float, Y : Float, PivotX : Float, PivotY : Float, Angle : Float, Point : FlxPoint = null) : FlxPoint
    {
        var sin : Float = 0;
        var cos : Float = 0;
    ...
    /**
		 ...
		 */
    public static function getAngle(Point1 : FlxPoint, Point2 : FlxPoint) : Float
    {
    ...
    /**
		 ...
		 */
    public static function getDistance(Point1 : FlxPoint, Point2 : FlxPoint) : Float
    {
