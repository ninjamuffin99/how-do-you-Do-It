...
private var ImgDefault : Class<Dynamic>;
    ...
    public function loadGraphic(Graphic : Class<Dynamic>, Animated : Bool = false, Reverse : Bool = false, Width : Int = 0, Height : Int = 0, Unique : Bool = false) : FlxSprite
    {
        ...
        if (Width == 0)
        {
            ...
            {
                Width = as3hx.Compat.parseInt(_pixels.width * 0.5);
            }
            else
            {
                Width = _pixels.width;
        ...
        if (Height == 0)
        {
            if (Animated)
            {
                Height = as3hx.Compat.parseInt(width);
            }
            else
            {
    ...
    //Create the brush and canvas
    {
        ...
        var rows : Int = Math.sqrt(Rotations);
        ...
        //Using just a segment of the graphic - find the right bit here{
            ...
            var rx : Int = as3hx.Compat.parseInt(Frame * brush.width);
            var ry : Int = 0;
            var fw : Int = full.width;
            if (rx >= fw)
            {
                ry = as3hx.Compat.parseInt(as3hx.Compat.parseInt(rx / fw) * brush.height);
                rx %= fw;
            }
            _flashRect.x = rx;
            _flashRect.y = ry;
        ...
        width = max * columns;
        height = max * rows;
        var key : String = Std.string(Graphic) + ":" + Frame + ":" + width + "x" + height;
        ...
        if (!skipGen)
        {
            ...
            var halfBrushWidth : Int = as3hx.Compat.parseInt(brush.width * 0.5);
            var halfBrushHeight : Int = as3hx.Compat.parseInt(brush.height * 0.5);
            var midpointX : Int = as3hx.Compat.parseInt(max * 0.5);
            var midpointY : Int = as3hx.Compat.parseInt(max * 0.5);
            while (row < rows)
            {
                column = 0;
    ...
    private function resetHelpers() : Void
    {
        ...
        frames = as3hx.Compat.parseInt((_flashRect2.width / _flashRect.width) * (_flashRect2.height / _flashRect.height));
    ...
    override public function draw() : Void
    {
        ...
        while (i < l)
        {
            ...
            _point.x = x - as3hx.Compat.parseInt(camera.scroll.x * scrollFactor.x) - offset.x;
            _point.y = y - as3hx.Compat.parseInt(camera.scroll.y * scrollFactor.y) - offset.y;
    ...
    //Draw line
    {
        ...
        var alphaComponent : Float = as3hx.Compat.parseFloat(as3hx.Compat.parseInt(Color >> 24) & 0xFF) / 255;
        if (alphaComponent <= 0)
        {
            alphaComponent = 1;
        }
    ...
    private function updateAnimation() : Void
    {
        if (_bakedRotation > 0)
        {
            ...
            var angleHelper : Int = as3hx.Compat.parseInt(angle % 360);
            if (angleHelper < 0)
            {
                angleHelper += 360;
            }
            _curIndex = as3hx.Compat.parseInt(angleHelper / _bakedRotation + 0.5);
    ...
    {
        _curAnim = null;
        _curIndex = as3hx.Compat.parseInt(FlxG.random() * (_pixels.width / frameWidth));
        dirty = true;
    }
    ...
    public function replaceColor(Color : Int, NewColor : Int, FetchPositions : Bool = false) : Array<Dynamic>
    {
        var positions : Array<Dynamic> = null;
        if (FetchPositions)
        {
    ...
    /**
		 ...
		 */
    private function get_facing() : Int
    {
        return _facing;
    }
    ...
    private function set_alpha(Alpha : Float) : Float
    {
        ...
        {
            _colorTransform = new ColorTransform((_color >> 16) * 0.00392, (as3hx.Compat.parseInt(_color >> 8) & 0xff) * 0.00392, (_color & 0xff) * 0.00392, _alpha);
        }
        else
        {
    ...
    {
        var indexX : Int = as3hx.Compat.parseInt(_curIndex * frameWidth);
        var indexY : Int = 0;
        ...
        {
            indexY = as3hx.Compat.parseInt(as3hx.Compat.parseInt(indexX / widthHelper) * frameHeight);
            indexX %= widthHelper;
        }
        ...
        {
            indexX = as3hx.Compat.parseInt((_flipped << 1) - indexX - frameWidth);
        }
