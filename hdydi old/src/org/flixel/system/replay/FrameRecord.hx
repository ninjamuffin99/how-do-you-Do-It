...
class FrameRecord
{
    ...
    {
        frame = as3hx.Compat.parseInt(Frame);
        keys = Keys;
        mouse = Mouse;
        return this;
    }
    ...
    public function load(Data : String) : FrameRecord
    {
        ...
        //get frame number
        var array : Array<Dynamic> = Data.split("k");
        frame = as3hx.Compat.parseInt(Std.string(array[0]));
        
        //split up keyboard and mouse data
        array = (Std.string(array[1])).split("m");
        ...
        //get keystroke data pairs{
            ...
            var keyPair : Array<Dynamic>;
            i = 0;
            l = array.length;
            while (i < l)
            {
                keyPair = (Std.string(array[i++])).split(":");
                if (keyPair.length == 2)
                {
                    if (keys == null)
                    {
                    ...
                    keys.push({
                                code : as3hx.Compat.parseInt(Std.string(keyPair[0])),
                                value : as3hx.Compat.parseInt(Std.string(keyPair[1]))
                            });
                }
            }
        }
        ...
        if (mouseData.length > 0)
        {
            ...
            if (array.length >= 4)
            {
                mouse = new MouseRecord(as3hx.Compat.parseInt(Std.string(array[0])), as3hx.Compat.parseInt(Std.string(array[1])), as3hx.Compat.parseInt(Std.string(array[2])), as3hx.Compat.parseInt(Std.string(array[3])));
            }
        }
