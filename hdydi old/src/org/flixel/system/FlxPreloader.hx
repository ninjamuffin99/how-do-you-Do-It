...
private var ImgLogoCorners : Class<Dynamic>;
    @:meta(Embed(source="../data/logo_light.png"))
private var ImgLogoLight : Class<Dynamic>;
    ...
    public function new()
    {
        ...
        {
            var re : as3hx.Compat.Regex = new as3hx.Compat.Regex('\\[.*:[0-9]+\\]', "");
    ...
    private function onEnterFrame(event : Event) : Void
    {
        ...
        if ((framesLoaded >= totalFrames) && (time > _min))
        {
            ...
            nextFrame();
            var mainClass : Class<Dynamic> = cast((Type.resolveClass(className)), Class);
            if (mainClass != null)
            {
    ...
    private function create() : Void
    {
        ...
        if (!FlxG.debug)
        {
            _min = as3hx.Compat.parseInt(minDisplayTime * 1000);
        }
        _buffer = new Sprite();
        _buffer.scaleX = 2;
        _buffer.scaleY = 2;
        addChild(_buffer);
        _width = as3hx.Compat.parseInt(stage.stageWidth / _buffer.scaleX);
        _height = as3hx.Compat.parseInt(stage.stageHeight / _buffer.scaleY);
