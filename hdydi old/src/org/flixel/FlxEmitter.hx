...
class FlxEmitter extends FlxGroup
{
    ...
    /**
		 ...
		 */
    public function makeParticles(Graphics : Class<Dynamic>, Quantity : Int = 50, BakedRotations : Int = 16, Multiple : Bool = false, Collide : Float = 0.8) : FlxEmitter
    {
        maxSize = Quantity;
        ...
        while (i < Quantity)
        {
            ...
            if (Multiple)
            {
                randomFrame = as3hx.Compat.parseInt(FlxG.random() * totalFrames);
                if (BakedRotations > 0)
                {
    ...
    {
        var particle : FlxParticle = try cast(recycle(FlxParticle), FlxParticle) catch(e:Dynamic) null;
