...
class FlxPath
{
    public static var manager(get, never) : DebugPathDisplay;
    ...
    /**
		 ...
		 */
    public function new(Nodes : Array<Dynamic> = null)
    {
        if (Nodes == null)
        {
    ...
    public function addAt(X : Float, Y : Float, Index : Int) : Void
    {
        ...
        as3hx.Compat.arraySplice(nodes, Index, 0, [new FlxPoint(X, Y)]);
    }
    ...
    public function addPointAt(Node : FlxPoint, Index : Int, AsReference : Bool = false) : Void
    {
        ...
        {
            as3hx.Compat.arraySplice(nodes, Index, 0, [new FlxPoint(Node.x, Node.y)]);
        }
    }
    ...
    public function removeAt(Index : Int) : FlxPoint
    {
        ...
        if (Index >= nodes.length)
        {
            Index = as3hx.Compat.parseInt(nodes.length - 1);
        }
        return nodes.splice(Index, 1)[0];
    }
    ...
    public function drawDebug(Camera : FlxCamera = null) : Void
    {
        ...
        //get a reference to the current node{
            ...
            node = try cast(nodes[i], FlxPoint) catch(e:Dynamic) null;
            
            //find the screen position of the node on this camera
            _point.x = node.x - as3hx.Compat.parseInt(Camera.scroll.x * debugScrollFactor.x);  //copied from getScreenXY()  
            _point.y = node.y - as3hx.Compat.parseInt(Camera.scroll.y * debugScrollFactor.y);
            _point.x = as3hx.Compat.parseInt(_point.x + (((_point.x > 0)) ? 0.0000001 : -0.0000001));
            _point.y = as3hx.Compat.parseInt(_point.y + (((_point.y > 0)) ? 0.0000001 : -0.0000001));
            ...
            _point.x = nextNode.x - as3hx.Compat.parseInt(Camera.scroll.x * debugScrollFactor.x);  //copied from getScreenXY()  
            _point.y = nextNode.y - as3hx.Compat.parseInt(Camera.scroll.y * debugScrollFactor.y);
            _point.x = as3hx.Compat.parseInt(_point.x + (((_point.x > 0)) ? 0.0000001 : -0.0000001));
            _point.y = as3hx.Compat.parseInt(_point.y + (((_point.y > 0)) ? 0.0000001 : -0.0000001));
    ...
    private static function get_manager() : DebugPathDisplay
    {
        return try cast(FlxG.getPlugin(DebugPathDisplay), DebugPathDisplay) catch(e:Dynamic) null;
    }
}
