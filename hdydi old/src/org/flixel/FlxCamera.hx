...
class FlxCamera extends FlxBasic
{
    ...
    /**
		 ...
		 */
    @:allow(org.flixel)
    private var _flashOffsetX : Float;
    /**
		 * Internal, used to render buffer to screen space.
		 */
    @:allow(org.flixel)
    private var _flashOffsetY : Float;
    /**
		 * Internal, used to render buffer to screen space.
		 */
    ...
    /**
		 ...
		 */
    public function follow(Target : FlxObject, Style : Int = STYLE_LOCKON) : Void
    {
        target = Target;
        var helper : Float;
        switch (Style)
    ...
    /**
		 ...
		 */
    public function flash(Color : Int = 0xffffffff, Duration : Float = 1, OnComplete : Function = null, Force : Bool = false) : Void
    {
        ...
        if (Duration <= 0)
        {
            Duration = as3hx.Compat.FLOAT_MIN;
        }
    ...
    /**
		 ...
		 * @param	Force		Force the effect to reset.
		 */
    public function fade(Color : Int = 0xff000000, Duration : Float = 1, OnComplete : Function = null, Force : Bool = false) : Void
    {
        ...
        _fxFadeAlpha = as3hx.Compat.FLOAT_MIN;
    }
    ...
    /**
		 ...
		 */
    public function shake(Intensity : Float = 0.05, Duration : Float = 0.5, OnComplete : Function = null, Force : Bool = true, Direction : Int = SHAKE_BOTH_AXES) : Void
    {
    ...
    private function set_color(Color : Int) : Int
    {
        ...
        colorTransform.greenMultiplier = (as3hx.Compat.parseInt(_color >> 8) & 0xff) * 0.00392;
    ...
    private function drawFX() : Void
    {
        ...
        if (_fxFlashAlpha > 0.0)
        {
            ...
            fill((as3hx.Compat.parseInt((((alphaComponent <= 0)) ? 0xff : alphaComponent) * _fxFlashAlpha) << 24) + (_fxFlashColor & 0x00ffffff));
        }
        ...
        if (_fxFadeAlpha > 0.0)
        {
            ...
            fill((as3hx.Compat.parseInt((((alphaComponent <= 0)) ? 0xff : alphaComponent) * _fxFadeAlpha) << 24) + (_fxFadeColor & 0x00ffffff));
        }
