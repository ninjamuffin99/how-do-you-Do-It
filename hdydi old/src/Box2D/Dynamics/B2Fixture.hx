...
class B2Fixture
{
	 ...
	 */
    private function Create(body : B2Body, xf : B2Transform, def : B2FixtureDef) : Void
    {
	 ...
	 */
    private function CreateProxy(broadPhase : IBroadPhase, xf : B2Transform) : Void
    //b2Assert(m_proxyId == b2BroadPhase::e_nullProxy);
    {
	 ...
	 * This supports body activation/deactivation.
	 */
    private function DestroyProxy(broadPhase : IBroadPhase) : Void
    {
        if (m_proxy == null)
        {
            return;
    ...
    private function Synchronize(broadPhase : IBroadPhase, transform1 : B2Transform, transform2 : B2Transform) : Void
    {
        if (!m_proxy)
        {
            return;
    ...
    private var m_density : Float;
    private var m_next : B2Fixture;
    private var m_body : B2Body;
    private var m_shape : B2Shape;
    
    private var m_friction : Float;
    private var m_restitution : Float;
    
    private var m_proxy : Dynamic;
    private var m_filter : B2FilterData = new B2FilterData();
    
    private var m_isSensor : Bool;
