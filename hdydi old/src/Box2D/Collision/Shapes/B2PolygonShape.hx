...
class B2PolygonShape extends B2Shape
{
    ...
    override public function Set(other : B2Shape) : Void
    {
        ...
        {
            var other2 : B2PolygonShape = try cast(other, b2PolygonShape) catch(e:Dynamic) null;
            m_centroid.SetV(other2.m_centroid);
            m_vertexCount = other2.m_vertexCount;
            cast((m_vertexCount), Reserve);
    ...
    public function SetAsVector(vertices : Array<B2Vec2>, vertexCount : Float = 0) : Void
    {
        ...
        for (i in 0...m_vertexCount)
        {
            ...
            b2Settings.b2Assert(edge.LengthSquared() >as3hx.Compat.FLOAT_MIN  /* * Number.MIN_VALUE*/  );
	...
	*/
    ...
    override public function RayCast(output : B2RayCastOutput, input : B2RayCastInput, transform : B2Transform) : Bool
    {
        ...
        // p = p1 + a * d{
            ...
            if (upper < lower - as3hx.Compat.FLOAT_MIN)
            {
                return false;
            }
        }
	...
	*/
    ...
    // Polygon mass, centroid, and inertia.
    {
        ...
        // Triangle vertices.{
            ...
            var p3 : B2Vec2 = (i + 1 < m_vertexCount) ? m_vertices[as3hx.Compat.parseInt(i + 1)] : m_vertices[0];
	...
	*/
    ...
    // Transform plane into shape co-ordinates
    {
        ...
        for (i in 0...m_vertexCount)
        {
            ...
            var isSubmerged : Bool = depths[i] < -as3hx.Compat.FLOAT_MIN;
            if (i > 0)
            {
                if (isSubmerged)
                {
                    if (!lastSubmerged)
                    {
                        intoIndex = as3hx.Compat.parseInt(i - 1);
                        diveCount++;
                    }
                }
                else if (lastSubmerged)
                {
                    outoIndex = as3hx.Compat.parseInt(i - 1);
                    diveCount++;
                }
            }
        ...
        switch (diveCount)
        {
            ...
            case 1:
                if (intoIndex == -1)
                {
                    intoIndex = as3hx.Compat.parseInt(m_vertexCount - 1);
                }
                else
                {
                    outoIndex = as3hx.Compat.parseInt(m_vertexCount - 1);
                }
        }
        var intoIndex2 : Int = as3hx.Compat.parseInt((intoIndex + 1) % m_vertexCount);
        var outoIndex2 : Int = as3hx.Compat.parseInt((outoIndex + 1) % m_vertexCount);
        ...
        while (i != outoIndex2)
        {
            i = as3hx.Compat.parseInt((i + 1) % m_vertexCount);
            if (i == outoIndex2)
            {
                p3 = outoVec;
            }
	...
	*/
    ...
    private var m_centroid : B2Vec2;
    
    private var m_vertices : Array<B2Vec2>;
    private var m_normals : Array<B2Vec2>;
    ...
    //b2Settings.b2Assert(count >= 3);
    {
        ...
        // Triangle vertices.{
            ...
            var p3 : B2Vec2 = (i + 1 < count) ? vs[as3hx.Compat.parseInt(i + 1)] : vs[0];
	 ...
	 */
    private static function ComputeOBB(obb : B2OBB, vs : Array<B2Vec2>, count : Int) : Void
    {
        var i : Int;
        ...
        var minArea : Float = as3hx.Compat.FLOAT_MAX;
        
        for (i in 1...count + 1)
        {
            var root : B2Vec2 = p[as3hx.Compat.parseInt(i - 1)];
            //b2Vec2 ux = p[i] - root;
            ...
            var lowerX : Float = as3hx.Compat.FLOAT_MAX;
            var lowerY : Float = as3hx.Compat.FLOAT_MAX;
            //b2Vec2 upper(-FLT_MAX, -FLT_MAX);
            var upperX : Float = -as3hx.Compat.FLOAT_MAX;
            var upperY : Float = -as3hx.Compat.FLOAT_MAX;
