...
class B2Simplex
{
            ...
            proxyB : B2DistanceProxy, transformB : B2Transform) : Void
    {
        ...
        if (m_count > 1)
        {
            ...
            if (metric2 < .5 * metric1 || 2.0 * metric1 < metric2 || metric2 < as3hx.Compat.FLOAT_MIN)
    ...
    public function WriteCache(cache : B2SimplexCache) : Void
    {
        ...
        for (i in 0...m_count)
        {
            cache.indexA[i] = as3hx.Compat.parseInt(vertices[i].indexA);
            cache.indexB[i] = as3hx.Compat.parseInt(vertices[i].indexB);
        }
    }
    ...
    public var m_vertices : Array<B2SimplexVertex> = new Array<B2SimplexVertex>();
