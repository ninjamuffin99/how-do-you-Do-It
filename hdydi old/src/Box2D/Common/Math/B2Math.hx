...
class B2Math
{
	...
	*/
    ...
    {
        var b : B2Vec2 = new B2Vec2(cast((a.x), Abs), cast((a.y), Abs));
        return b;
    }
    
    public static function AbsM(A : B2Mat22) : B2Mat22
    {
        var B : B2Mat22 = b2Mat22.FromVV(cast((A.col1), AbsV), cast((A.col2), AbsV));
        return B;
    }
    ...
    {
        x = x | as3hx.Compat.parseInt(as3hx.Compat.parseInt(x >> 1) & 0x7FFFFFFF);
        x = x | as3hx.Compat.parseInt(as3hx.Compat.parseInt(x >> 2) & 0x3FFFFFFF);
        x = x | as3hx.Compat.parseInt(as3hx.Compat.parseInt(x >> 4) & 0x0FFFFFFF);
        x = x | as3hx.Compat.parseInt(as3hx.Compat.parseInt(x >> 8) & 0x00FFFFFF);
        x = x | as3hx.Compat.parseInt(as3hx.Compat.parseInt(x >> 16) & 0x0000FFFF);
        return as3hx.Compat.parseInt(x + 1);
    }
    
    public static function IsPowerOfTwo(x : Int) : Bool
    {
        var result : Bool = x > 0 && (x & as3hx.Compat.parseInt(x - 1)) == 0;
        return result;
    }
